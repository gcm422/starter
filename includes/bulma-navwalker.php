 ...
 ************/
     ...
     */
		...
		public function end_lvl( &$output, $depth = 0, $args = array() ) {
			$indent = str_repeat( "\t", $depth );
			$output .= "$indent</ul></li></ul><!-- END LEVEL -->\n"; /* modificato da samu*/
		}
			...
			if ( ! $args['has_children'] ) {//if doesn't have children
						if ( ! in_array( 'navbar-divider', $classes ) ) {//if doesnt contains a navbar divider
							...
							if ( ! empty( $fa ) ) {//if item title is not empty
										$item_output .= '<li' . $class_names . '><a class="navbar-link"'. $attributes . '><i class="' . $fa . '"></i></a>';
										if (is_bool($the_title)) {
													$show_title = false;//don't display text
										}
							}
							else{
								$item_output .= '<li' . $class_names . '><a class="navbar-item"' . $attributes . '>';//item empty use defaults
							}
				...
				$item_output .= $indent . '<ul class="navbar-item has-dropdown is-hoverable" data-target="dropdown"><!-- START DROPDOWN-->' . "\n";
					if ( ! empty( $fa ) ) {//if fa  is not empty
						$item_output .= '<li' . $class_names . '><a class="navbar-link"' . $attributes . '><i class="' . $fa . '"></i>';
						...
						$item_output .= '<li' . $class_names . '><a class="navbar-link"' . $attributes . $id . '>';//output standard
					}
     ...
     */
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				echo '>';
				echo '<li class="navbar-item"><a class="navbar-link" href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="">' . esc_attr( 'Add a menu', 'worldoweb' ) . '</a></li>';
				echo '</ul>';
				if ( $container ) {
					echo '</' . esc_attr( $container ) . '>';
				}
